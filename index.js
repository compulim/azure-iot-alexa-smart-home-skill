...
const debug   = require('debug')('alexa');
...
function handleControl(event) {
  ...
  case 'SetPercentageRequest':
    return handleSetPercentageRequest(event.payload);

  case 'IncrementPercentageRequest':
    return handleIncrementPercentageRequest(event.payload);

  case 'DecrementPercentageRequest':
    return handleDecrementPercentageRequest(event.payload);
  }
}

function handleTurnOnRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'turnOn', {}, 'TurnOnConfirmation');
}

function handleTurnOffRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'turnOff', {}, 'TurnOffConfirmation');
}

function transformTemperatureDeviceResponse(json) {
  return {
    targetTemperature: json.value,
    temperatureMode  : json.mode,
    previousState    : {
      mode             : json.prevMode,
      targetTemperature: json.prevValue
    }
  };
}

function handleSetTemperatureRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'setTemperature', { value: payload.targetTemperature.value }).then(transformTemperatureDeviceResponse);
}

function handleIncrementTemperatureRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'incrementTemperature', { delta: payload.deltaTemperature.value }).then(transformTemperatureDeviceResponse);
}

function handleDecrementTemperatureRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'decrementTemperature', { delta: payload.deltaTemperature.value }).then(transformTemperatureDeviceResponse);
}

function handleSetPercentageRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'setPercentage', { value: payload.percentageState.value });
}

function handleIncrementPercentageRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'incrementPercentage', { delta: payload.deltaPercentage.value });
}

function handleDecrementPercentageRequest(payload) {
  return invokeDeviceMethod(payload.appliance.applianceId, 'decrementPercentage', { delta: payload.deltaPercentage.value });
}
...
function handleHealthCheckRequest(payload) {
  // TODO: Should health check every device
  return {
    header: {
